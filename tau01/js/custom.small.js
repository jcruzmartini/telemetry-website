var configurator = {};
configurator.wsUrl = "http://ec2-54-213-29-133.us-west-2.compute.amazonaws.com/tau-services";
Tau = {};
Tau.Configurator = {};
Tau.AuthorizationToken = 'cacuhgay';
...
function sparkline_charts() {
	...
	var chartColours = ['#2FABE9', '#FA5833', '#77AC34', '#bbdce3', '#9a3b1b', '#5a8022', '#2c7282'];
	...
	for (i = 1; i < 9; i++) {
		...
		if (retina()) {
			...
			$(placeholder).sparkline(data, {
				...
				maxSpotColor: '#77AC34', //The CSS colour of the marker displayed for the maximum value. Set to false or an empty string to hide it
...
function showLogin() {
	...
	$("#login-email-form-ingresar").click(function () {
		...
		if (invalid) {
			...
			$.ajax({
				type: 'POST',
				url: configurator.wsUrl + "/users/login",
				contentType: 'application/json; charset=utf-8',
				data: JSON.stringify(jsonrequest),
				dataType: "json",
				success: function (rta) {
					if (rta.success) {
						var userData2 = rta.result;
						$('#login-email-form').modal('hide');
						$("#user").html("&nbsp;" + userData2.email);
						Tau.AuthorizationToken = rta.result.token;
						Cookie.Set('userData', JSON.stringify(userData2));
						var i = 0;
						$.each(userData2.stations, function (i, fb) {
							if (fb.is_default == 1) {
								Cookie.Set('activeStation', JSON.stringify(fb));
							}
							i++;
...
function startApp() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			var activeStation = jQuery.parseJSON(Cookie.Get('activeStation'));


			$.ajax({
				type: 'GET',
				url: configurator.wsUrl + "/measures/" + activeStation.id + "/actual?eToken=" + encodeURIComponent(Tau.AuthorizationToken),
				dataType: "json",
				success: function (rta) {
					if (rta.success) {
			...
			$.ajax({
				type: 'GET',
				url: configurator.wsUrl + "/stations/" + activeStation.id + "/info?eToken=" + encodeURIComponent(Tau.AuthorizationToken),
				dataType: "json",
				success: function (rta) {
					if (rta.success) {
			...
			$.ajax({
				type: 'GET',
				url: configurator.wsUrl + "/measures/" + activeStation.id + "/summary?eToken=" + encodeURIComponent(Tau.AuthorizationToken),
				dataType: "json",
				success: function (rta) {
					if (rta.success) {
						var lines = "";
						...
						$.each(rta.result.entries, function (i, o) {
							...
							if (typeof o.variable.unit == 'undefined') {
								o.variable.unit = 'N/A';
							};
							if (typeof o.values[0].min == 'undefined') {
								o.values[0].min = '-';
							};
							if (typeof o.values[0].max == 'undefined') {
								o.values[0].max = '-';
							};
							if (typeof o.values[0].avg == 'undefined') {
								o.values[0].avg = '-';
							};

							if (typeof o.values[1].min == 'undefined') {
								o.values[1].min = '-';
							};
							if (typeof o.values[1].max == 'undefined') {
								o.values[1].max = '-';
							};
							if (typeof o.values[1].avg == 'undefined') {
								o.values[1].avg = '-';
							};

							if (typeof o.values[2].min == 'undefined') {
								o.values[2].min = '-';
							};
							if (typeof o.values[2].max == 'undefined') {
								o.values[2].max = '-';
							};
							if (typeof o.values[2].avg == 'undefined') {
								o.values[2].avg = '-';
							};
							lines += "<tr><td rowspan='3'><strong>" + o.variable.name + "</strong> <br />(" + o.variable.unit + ")</td>" +
								"<td style='text-align:center'><strong>Diario</strong></td>" +
								"<td style='text-align:center'>" + o.values[0].min + "</td>" +
								"<td style='text-align:center'>" + o.values[0].max + "</td></tr>" +
								"<tr><td style='text-align:center'><strong>Mensual</strong></td>" +
								"<td style='text-align:center'>" + o.values[1].min + "</td>" +
								"<td style='text-align:center'>" + o.values[1].max + "</td></tr>" +
								"<tr><td style='text-align:center'><strong>Anual</strong></td>" +

							"<td style='text-align:center'>" + o.values[2].min + "</td>" +
								"<td style='text-align:center'>" + o.values[2].max + "</td>" +
								"</tr>";
						});
						var tabla = "<table class='table table-bordered table-striped table-condensed'>";
						tabla += "<thead> <tr><th>Variable</th><th>Per&iacute;odo</th><th>M&iacute;nimo</th><th>M&aacute;ximo</th></thead>";
			...
			$.ajax({
				type: 'GET',
				url: configurator.wsUrl + "/entity/" + activeStation.id + "/variable/all?eToken=" + encodeURIComponent(Tau.AuthorizationToken),
				dataType: "json",
				success: function (rta) {
					if (rta.success) {
						$.each(rta.result.entries, function (i, o) {
							listOfVar = listOfVar + '' + o.code + ',';
						});
						var initDate = date2ts(new Date())-46800000;
						var endDate = date2ts(new Date());
						var url = configurator.wsUrl + "/measures/" + activeStation.id + "/historical?eToken=" + encodeURIComponent(Tau.AuthorizationToken)+ "&initDate="+initDate+"&endDate="+endDate+"&codes="+listOfVar;
						$.ajax({
							type: 'GET',
							url: url,
							dataType: "json",
							success: function (rta) {
								if (rta.success) {
									...
									var tabla = "<table id='tabla-diaria' class='table table-bordered table-striped table-condensed' style='width:100%;'>";
									...
									$.each(rta.result.entries, function (i, o) {
										...
										$.each(o.values, function (j, o1) {
											if (j1 < 4) {
												...
												line += '<td style="text-align:center;">' + parseFloat(o1.value).toFixed(1) + '</td>';
											}
											j1++;
										});
										...
										if (j1 == 1) {
											...
											for (var i = 0; i < 4 - j1; i++) {
												//line += '<td>&nbsp;</td>';
											}
										}
										lines += '<tr><td><strong>' + o.variable.name + '</strong><br /> (' + o.variable.unit + ')</td>' + line + '</tr>';
			...
			$.ajax({
				...
				success: function (rta) {
					if (rta.success) {
						var i = 0;
						j = 0;
						var data = '<table width="100%" style="width:100%;"><tr>';
						var windvelocity = '';
						windmax = '';
						windmin = '';
						$('#tempmin-tr').hide();
						...
						$.each(rta.result.entries, function (i, o) {
							...
							if (o.variable.code == 'L') {
								$("#precipitaciones").html(o.values[0].value);
								$("#precipitaciones-unidad").html(" " + o.variable.unit);
								...
								if (o.variable.code == 'MT') {
									...
									$('#evapo-tr').show();
									$('#evapo-span').html(o.values[0].value + " " + o.variable.unit);
								} else if (o.variable.code == 'W') {
								...
								} else if (o.variable.code == 'DV') {
									j++;
									if (j == 4) data += '</tr><tr>';
									data += '<td width="13%"><h3 title="' + o.variable.tooltip + '">' + o.variable.name + '</h3><div style="left:50%"><div class="circleStatsItem green">';
									data += '<i><img src="./img/sensors/green/' + o.variable.filename + '" width="40%"></i>';
									data += '<span class="percent" style="left:-15px;">' + windvelocity + '</span>';
									data += '<input type="text" id="viento-text" value="" style="background-color:transparent;font-weight:bold;font-size:75%;"/>';
									data += '<input type="text" id="viento-number" value="' + o.values[0].value + '" class="greenCircle3 smallcircle" />';
									data += '</div>';
									data += '</div>';
									data += '<span class="badge badge-info" style="width:54px;" title="Valor M&iacute;nimo Diario">' + windmin + '</span> <br /><span class="badge badge-important" style="width:54px;" title="Valor M&aacute;ximo Diario">' + windmax + '</span>';
									data += '</td>';
								} else {
									j++;
									if (j == 4) data += '</tr><tr>';
									...
									if (o.variable.code == 'P') {
										...
										data += '<input type="text" value="' + o.values[0].value + '" style="font-size:75%;" class="greenCircle2" />';
									} else if (o.variable.code == 'R') {
										data += '<span class="percent" style="left:-18px;">' + o.variable.unit + '</span>';
										data += '<input type="text" value="' + o.values[0].value + '" style="font-size:75%;" class="greenCircle smallcircle" />';
									} else {
										data += '<span class="percent">' + o.variable.unit + '</span>';
										data += '<input type="text" value="' + o.values[0].value + '" style="font-size:75%;" class="greenCircle smallcircle" />';
									}
									data += '</div>';
									data += '</div>';
									data += '<span class="badge badge-info" style="width:54px;" title="Valor M&iacute;nimo Diario">' + o.values[0].min + ' <span style="font-size:80%">' + o.variable.unit + '</span></span><br /><span class="badge badge-important" style="width:54px;" title="Valor M&aacute;ximo Diario">' + o.values[0].max + ' <span style="font-size:80%">' + o.variable.unit + '</span></span>';
									data += '</td>';
								}
						...
						//$("#circles-div").children("canvas").animate({width:'10px'}, 1200);
						circle_progess();
			...
			$.ajax({
				type: 'GET',
				url: configurator.wsUrl + "/notifications/" + activeStation.id + "/active?eToken=" + encodeURIComponent(Tau.AuthorizationToken),
				dataType: "json",
				success: function (rta) {
					if (rta.success) {
						var delay;
